...
public protocol UILabelClickableLinksDelegate: NSObjectProtocol {
    func clickableLabel(_ label: UILblClickableLinks, didTapUrl url: String, atRange range: NSRange)
}

public class UILblClickableLinks: UILabel {
    ...
    public weak var delegate: UILabelClickableLinksDelegate?
    ...
    public override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for (urlString, range) in links where NSLocationInRange(indexOfCharacter, range) {
            delegate?.clickableLabel(self, didTapUrl: urlString, atRange: range)
            return
        }
    }
